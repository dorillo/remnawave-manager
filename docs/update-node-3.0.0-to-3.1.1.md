# Обновление Node до 3.4.0

Целевой образ Node `3.4.0` привязан к immutable multiarch digest в `compatibility.json`. Обновление меняет только прямое поле `image` сервиса Node в Compose и, для старого стека, исправляет target лога `/var/log/remnanode` на `/var/log/xray`.

Перед обновлением Node обязательно обновите и проверьте Panel `3.4.1`. Node и Panel должны использовать один актуальный API-контракт; менеджер остановит переход со старой Node без явного `--panel-3-4-ready`.

Node 3.4.0 сохраняет Xray Core `v26.7.28`, runtime-команды `rw-core`/`cli` и Unix-сокеты XHTTP. Версия добавляет необязательные `NFTABLES_LOGGING`, `NFTABLES_ACCEPT_REPLY_TRAFFIC` и `SNI_VERIFICATION`; старый env остаётся валидным, а чистая установка задаёт upstream-compatible defaults явно. Менеджер проверяет `SECRET_KEY` и текущий Xray JSON новым образом до изменения Compose; Reality без явного `minClientVer` требует явного подтверждения риска.

Если старый `SECRET_KEY` принят предыдущей Node, но отклонён 3.4.0, интерактивный `rwm update` попросит новый ключ, выданный обновлённой Panel через `/api/keygen` (или скопированный из настроек Node). Ключ сначала проходит preflight образом 3.4.0, затем менеджер транзакционно заменяет его в `.env` либо в прямом `environment:` сервиса Node и обновляет Compose. Ввод принимает чистый ключ, строку `SECRET_KEY=…`/`SECRET_KEY: …` из Compose и JSON-ответ `/api/keygen`; перед сохранением оболочка удаляется. Для неинтерактивного запуска передайте новый ключ только через окружение: `RWM_NODE_SECRET_KEY='…' sudo --preserve-env=RWM_NODE_SECRET_KEY rwm update ...`; после запуска удалите переменную.
...
sudo rwm diagnose
sudo rwm backup create --reason before-node-3.4.0
