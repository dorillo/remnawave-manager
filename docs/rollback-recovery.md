...
Для выборочного необратимого удаления укажите один или несколько архивов:

```bash
sudo rwm backup delete ИМЯ_BACKUP.tar.gz ДРУГОЙ_BACKUP.tar.gz
```

CLI потребует подтвердить операцию вводом `y`; для автоматизации можно явно
добавить `--yes`. В интерактивном разделе backup менеджер показывает нумерованный список
и принимает номера через пробел или запятую. Удаление разрешено только для
обычных single-link `.tar.gz` непосредственно из каталога backup и блокируется,
пока существует journal незавершённой транзакции.
...
`backup list`, ручное удаление и retention рассматривают только обычные single-link `.tar.gz`; symlink, hardlink и специальные файлы игнорируются или отклоняются. Перед удалением менеджер повторно проверяет inode и метаданные, перемещает выбранный архив в случайный quarantine и удаляет его только после повторной проверки. Подмена останавливает удаление, сохраняя подозрительный файл в quarantine. `--retention` принимает от 1 до 1000. Dry-run отсутствует.
...
CLI потребует подтвердить восстановление вводом `y`. Для автоматизации существует `--yes`, но он только отключает подтверждение.
