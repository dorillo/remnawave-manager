...
class CliDispatchTests(unittest.TestCase):
    ...
    def test_node_update_retries_manual_secret_without_panel_api(self) -> None:
        backup = BackupResult(Path(self.temporary.name) / "pre-node.tar.gz", {})
        with (
            mock.patch(
                "remnawave_manager.cli.StateStore.load_inventory",
            ...
            mock.patch("remnawave_manager.cli.assert_no_active_certbot_renewal"),
            mock.patch("remnawave_manager.cli.RemnawaveApi") as api_type,
            mock.patch(
                "remnawave_manager.cli.update_node",
                side_effect=[
            ...
            code = self.run_main(
                ["update", "--yes"],
                answers=["y"],
                secrets=["copied-node-secret", "valid-node-secret"],
            )

        self.assertEqual(code, 0, self.stderr.getvalue())
        api_type.assert_not_called()
        self.assertEqual(update.call_count, 3)
        self.assertEqual(update.call_args.kwargs["replacement_secret"], "valid-node-secret")
        output = self.stdout.getvalue() + self.stderr.getvalue()
        self.assertNotIn("copied-node-secret", output)
        self.assertNotIn("valid-node-secret", output)

    def test_node_update_can_cancel_after_invalid_manual_secret(self) -> None:
        with (
            mock.patch(
                "remnawave_manager.cli.StateStore.load_inventory",
                return_value=inventory("node"),
            ...
            code = self.run_main(
                ["update", "--yes"],
                answers=["n"],
                secrets=["copied-node-secret"],
            )

        self.assertEqual(code, 2)
        self.assertEqual(update.call_count, 2)
        api_type.assert_not_called()
        self.assertIn("отменено", self.stderr.getvalue())
