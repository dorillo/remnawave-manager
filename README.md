...
Запускайте установщик из доверенной локальной копии репозитория:
...
Сам менеджер обновляется повторным запуском `sudo ./install.sh` из новой проверенной локальной копии репозитория. Установщик собирает и проверяет отдельное Python-окружение, атомарно переключает `/usr/local/bin/rwm` только после успешной проверки и не загружает исполняемый код по плавающей ссылке.
