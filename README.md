...
Менеджер никогда не следует плавающему тегу `latest` и не повышает версии только потому, что upstream опубликовал новый релиз. Новая версия становится поддерживаемой после отдельной проверки release notes, миграций, API-контрактов и digest образов, затем явно добавляется в compatibility manifest. Локальные тесты проверяют контракты API, генераторы и сценарии управления; проверка реальных контейнеров, миграций данных и сетевого трафика требует Ubuntu 24.04 с Docker.

Целевая версия — [Panel 3.4.4](https://github.com/remnawave/backend/releases/tag/3.4.4), официальный API-контракт — `@remnawave/backend-contract@3.4.15`. Версии 3.4.0-3.4.1 добавляют штатные Prisma-миграции для режимов привязки Host к Internal Squads и тегов сущностей, а также необязательные `SHORT_UUID_METHOD` и `SHORT_UUID_CUSTOM_PATTERN`. Версия 3.4.2 исправляет конкурентную регистрацию HWID, а 3.4.3 закрывает обход аутентификации backend-tools через смешанный регистр пути. Panel 3.4.4 исправляет расчёт сброса трафика и обновление нод; до этой версии включительно после 3.4.1 новых миграций и обязательных переменных окружения нет; старый `.env` остаётся валидным. При чистой установке менеджер явно выбирает совместимые `nanoid` и длину 16. Multiarch manifest digest закреплён в Docker Hub/GHCR.

Node 3.4.1 сохраняет Xray Core `v26.7.28`, ENV-контракт, runtime-команды и Unix-сокеты XHTTP версии 3.4.0. Релиз обновляет Node Plugins/Zod и исправляет сброс старых соединений при замене VLESS UUID. Менеджер использует проверенную связку с Panel 3.4.4: сначала обновляется и проверяется Panel; CLI требует явное `--panel-3-4-ready` при переходе со старой Node. Перед переключением менеджер проверяет строгий контракт `SECRET_KEY` 3.4.1 и текущий Xray JSON. Если старый ключ несовместим, интерактивное обновление запросит полный `SECRET_KEY` из конфигурации нужной Node в Panel и проверит его до изменения `.env` или Compose.

| Компонент | Проверенная исходная версия | Целевая версия |
| --- | --- | --- |
| Remnawave Panel | 2.8.1, 3.0.0, 3.1.0, 3.2.0, 3.2.1, 3.2.2, 3.2.3, 3.3.0, 3.3.1, 3.3.2, 3.4.0, 3.4.1, 3.4.2, 3.4.3, 3.4.4 | 3.4.4 |
| Subscription Page | 7.2.6, 8.0.0 | 8.0.0 |
| Remnawave Node | 2.8.0, 3.0.0, 3.1.0, 3.1.1, 3.2.0, 3.2.1, 3.2.2, 3.3.0, 3.3.1, 3.3.2, 3.4.0, 3.4.1 | 3.4.1 |
...
Если обычная загрузка зависает или завершается ошибкой HTTP/2, используйте резервную команду через HTTP/1.1:

```bash
curl --http1.1 -fsSL --connect-timeout 15 --max-time 60 https://raw.githubusercontent.com/dorillo/remnawave-manager/main/install.sh | sudo bash -s -- install --http1.1
```

Параметр у `curl` задаёт режим скачивания установщика, а `install --http1.1` — скачивания архива менеджера и DNS-плагина Gcore внутри установщика. Оба параметра нужны, чтобы выбранный режим действовал на всех этих загрузках. HTTP/1.1 может помочь при сбоях HTTP/2, но не обходит блокировку доступа к GitHub.
...
В интерактивном терминале главное и вложенные меню очищают предыдущий экран перед отрисовкой. После выполнения операции результат остаётся видимым до нажатия Enter, затем снова открывается текущий раздел; в главное меню возвращает только пункт `0. Назад`. При перенаправлении вывода и в `--json` управляющие последовательности не используются.
...
Менеджер установит Panel 3.4.4 и Subscription Page 8.0.0 на одном сервере, настроит nginx, UFW, TLS и защитный URL с cookie. Node при этом не устанавливается.
...
На сервере Node выберите один из семи встроенных шаблонов:
...
Вместо `--template` можно указать подготовленный локальный каталог через `--site-source`. На Linux каталог, его содержимое и все родители должны принадлежать root и не допускать group/world write; это защищает root-копирование от подмены дерева. Случайного выбора заглушки нет. Для встроенных сайтов используйте `--template`: произвольный `--site-source` копируется как готовый самостоятельный сайт, без автоматической настройки его API-прокси.
...
- [обновление Panel до 3.4.4 и Subscription Page 8.0.0](docs/migration-panel-3.2.1-to-3.2.3.md);
- [обновление Node до 3.4.1](docs/update-node-3.0.0-to-3.1.1.md);
...
Ненужные архивы можно выбрать по номерам в разделе «Резервные копии и
восстановление» интерактивного меню. Допускаются один номер или несколько
номеров через пробел либо запятую. Для автоматизации доступны абсолютные пути
и имена файлов:

```bash
sudo rwm backup delete ИМЯ_BACKUP.tar.gz ДРУГОЙ_BACKUP.tar.gz
```

Команда показывает подтверждение `(y/n)` и выполняется необратимо. Она
принимает только архивы непосредственно из `/var/backups/remnawave-manager`,
которые присутствуют в безопасном списке менеджера, повторно проверяет их перед
удалением и не запускается при незавершённой транзакции.
...
`--retention` удаляет только повторно проверенные более старые локальные архивы той же роли и не является dry-run. Ручное удаление использует такую же проверку inode и метаданных с изоляцией архива перед окончательным удалением. Restore сначала создаёт приватную стабильную копию сжатого архива, поэтому до распаковки требуется как минимум размер `.tar.gz` плюс `256 MiB` свободного места, а staging содержимого может потребовать больше.
...
Доступны семь сайтов: [Line](docs/northline.md) (`01-northline`),
[Aster](docs/aster.md) (`02-aster-observatory`), [Morrow](docs/morrow.md)
(`03-morrow-coffee`), [Spros](docs/spros.md) (`04-signal-works`),
[Svod](docs/svod.md) (`05-field-notes`), [Loop](docs/loop.md) (`06-loop-archive`)
и [Fokus](docs/fokus.md) (`07-fokus-news`). Код и оформление поставляются локально;
публичный контент читается через ограниченные nginx-маршруты. Личные данные
хранятся в браузере. Серверное приложение для сайтов не требуется.

Сайты 8–10 удалены из поставки и выбора. Уже установленный старый сайт можно
заменить любым из семи; предварительно создаётся backup. Работающий nginx
точечно пересоздаётся, остановленный остаётся остановленным. Node, WARP и
Subscription Page не пересоздаются. Условия для существующих конфигураций,
CDN, XHTTP и восстановления — в [руководстве](docs/node-sites.md).

```bash
sudo rwm disguise apply 03-morrow-coffee
```

Для сервера с несколькими IPv4 можно выбрать исходящий адрес внешних запросов
сайта: **Сайты-заглушки Node → Исходящий IP сайта** или
`sudo rwm disguise egress set IP`. Есть проверка источника через выбранный адрес,
backup и автоматический откат. Настройка сохраняется при смене шаблона;
Xray, WARP и системные маршруты не меняются. [Подробнее](docs/site-egress.md).

Для локального просмотра используйте соответствующий `scripts/preview_*.py`:
он раздаёт сайт и ограниченные API-маршруты. Команды для всех семи сайтов
приведены в [документации прокси](docs/disguise-image-proxy.md).
Обычный Live Server не создаёт эти маршруты.
...
Сам менеджер обновляется отдельным пунктом меню или командой:

```bash
sudo rwm manager update
```

В пункте **17. Обновить Remnawave Manager** доступны два варианта: обычное обновление с автоматическим выбором версии HTTP и резервное обновление через HTTP/1.1. Пункт `0` возвращает в главное меню. При ошибке можно выбрать другой вариант; после успешного обновления менеджер завершает работу.

Резервный вариант из командной строки:

```bash
sudo rwm manager update --http1.1
```

Выбранный режим применяется и к загрузке `install.sh`, и к загрузкам внутри установщика. В обоих режимах скачивание установщика и архива ограничено таймаутами; при скачивании архива отображается прогресс.

Команда загружает `install.sh` из `dorillo/remnawave-manager` (`main`) в защищённый временный файл, проверяет его формат и запускает без shell-конкатенации. Установщик собирает и проверяет отдельное Python-окружение и атомарно переключает `/usr/local/bin/rwm` только после успешной проверки. После успеха старый процесс меню завершается. Для воспроизводимой установки с предварительным аудитом по-прежнему используйте `sudo ./install.sh install` из локальной копии конкретного коммита.
...
- [Сайты Node: установка, замена и совместимость](docs/node-sites.md)
- [WARP](docs/warp.md)
...
TMPDIR="$PWD/tests/.tmp/runtime" PYTHONPATH=src python3 -m unittest discover -s tests
```

Приватный временный каталог внутри проекта нужен проверкам безопасных путей, в том числе на macOS. Тесты диагностики имитируют root-владение там, где проверяется другой сценарий; тест установки Node использует временный каталог вместо Linux `/dev/shm`. Это не проверка Docker, systemd или сетевого трафика на сервере. Тесты реальной смены владельца без root пропускаются.

Дополнительная проверка запросов менеджера по официальным схемам Panel 3.4.4 требует Node.js и npm. Пакет устанавливается только в игнорируемый тестовый каталог и не является зависимостью менеджера:

```bash
npm install --prefix tests/.tmp/upstream-contract --ignore-scripts --no-package-lock --no-audit --no-fund @remnawave/backend-contract@3.4.15
RWM_BACKEND_CONTRACT_PATH="$PWD/tests/.tmp/upstream-contract/node_modules/@remnawave/backend-contract" \
  TMPDIR="$PWD/tests/.tmp/runtime" PYTHONPATH=src \
  python3 -m unittest discover -s tests -p test_upstream_contract.py -v
```

Без `RWM_BACKEND_CONTRACT_PATH` этот дополнительный тест пропускается; остальные тесты не требуют npm или доступа к сети.
...
## Исправления сайтов и меню в 0.1.23

- Line и Aster: одинаковые кнопки языка и темы 44×44 px, SVG-иконка солнца;
  сетка загрузки Aster повторяет число колонок ленты на мобильных и широких экранах.
- Loop: поддержка строковых ID GIFS.RU в ленте, карточках и похожих медиа.
  Сохраняется совместимость с числовыми ID; nginx разрешает только ограниченный
  буквенно-цифровой идентификатор. Известные предыдущие маршруты обновляются
  с сохранением исходящего IP сайта и VPN locations.
- Меню исходящего IP остаётся открытым после команды, включая ошибку.
  «Назад» в выборе IP, шаблона и backup возвращает на один уровень вверх.

Обновление менеджера само по себе не заменяет уже установленный сайт.
Чтобы применить исправления, повторно выберите текущий шаблон в
«Сайты-заглушки Node → Установить шаблон»; для Loop это также обновит nginx-маршрут
карточки. Перед применением менеджер создаёт backup. Проверка источника через IP
показывает сетевую доступность API, но не заменяет проверку сайта в браузере.


## Исходящий IPv4 сайта в 0.1.24

При выборе конкретного IPv4 внешние маршруты сайта используют отдельные nginx
upstream-группы с `resolve` и `ipv6=off`. DNS-серверы берутся из `/etc/resolv.conf`,
адреса источников обновляются по DNS TTL; глобальные DNS/IPv6, VPN и режим `auto`
не меняются. Требуется nginx **1.27.3 или новее** (штатный образ менеджера — 1.28).
Host, TLS SNI, проверка сертификатов, URI и параметры запросов сохраняются.

Проверка текущего исходящего IP теперь включает повторные запросы через настоящий
nginx и, если ответ источника содержит подходящие данные, публикацию/карточку и
образец медиа. Для другого IP проверяется прямой доступ; проверка nginx выполняется
после применения. Ошибка любой проверки после применения вызывает откат.
Для Spros исправлен проверочный запрос: он передаёт обязательный `limit=20`,
чтобы штатный nginx-маршрут не отклонял его с HTTP 400.
Для Yappy сохранён прежний объём проверки: по одному запросу к двум страницам
ленты, без повторения каждой страницы. HTTP 403/429 прекращает дальнейшие
запросы через nginx, чтобы проверка не усиливала ограничения источника.

Чтобы обновить существующую настройку, повторно выберите **тот же IP** в
«Сайты-заглушки Node → Исходящий IP сайта → Изменить IP». Менеджер распознаёт
старые привязки и обновляет их с backup, проверкой nginx и сохранением IP.
Обновление пакета менеджера само по себе не изменяет рабочую конфигурацию.
