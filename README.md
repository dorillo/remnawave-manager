...
Для Panel 3.4.3 проверены официальные release notes и полный diff backend с 3.3.2. Версии 3.4.0-3.4.1 добавляют штатные Prisma-миграции для режимов привязки Host к Internal Squads и тегов сущностей, а также необязательные `SHORT_UUID_METHOD` и `SHORT_UUID_CUSTOM_PATTERN`. Версия 3.4.2 исправляет конкурентную регистрацию HWID, а 3.4.3 закрывает обход аутентификации backend-tools через смешанный регистр пути. После 3.4.1 новых миграций и обязательных переменных окружения нет; старый `.env` остаётся валидным. При чистой установке менеджер явно выбирает совместимые `nanoid` и длину 16. Multiarch manifest digest закреплён в Docker Hub/GHCR.

Node 3.4.1 сохраняет Xray Core `v26.7.28`, ENV-контракт, runtime-команды и Unix-сокеты XHTTP версии 3.4.0. Релиз обновляет Node Plugins/Zod и исправляет сброс старых соединений при замене VLESS UUID. Поэтому Node обновляется только после Panel 3.4.3; CLI требует явное `--panel-3-4-ready` при переходе со старой Node. Перед переключением менеджер проверяет строгий контракт `SECRET_KEY` 3.4.1 и текущий Xray JSON. Если старый ключ несовместим, интерактивное обновление запросит полный `SECRET_KEY` из конфигурации нужной Node в Panel и проверит его до изменения `.env` или Compose.

| Компонент | Проверенная исходная версия | Целевая версия |
| --- | --- | --- |
| Remnawave Panel | 2.8.1, 3.0.0, 3.1.0, 3.2.0, 3.2.1, 3.2.2, 3.2.3, 3.3.0, 3.3.1, 3.3.2, 3.4.0, 3.4.1, 3.4.2, 3.4.3 | 3.4.3 |
...
Менеджер установит Panel 3.4.3 и Subscription Page 8.0.0 на одном сервере, настроит nginx, UFW, TLS и защитный URL с cookie. Node при этом не устанавливается.
...
- [обновление Panel до 3.4.3 и Subscription Page 8.0.0](docs/migration-panel-3.2.1-to-3.2.3.md);
...
Доступные ID заглушек: `01-northline`, `02-aster-observatory`, `03-morrow-coffee`, `04-signal-works`, `05-field-notes`, `06-loop-archive`, `07-fokus-news`, `08-vector-docs`, `09-pulse-monitor`, `10-dev-circle`. Шаблоны хранят код и оформление локально. Линия, Астер, Морроу и Спрос используют внешние источники контента; Морроу показывает короткие видео Yappy, а [Спрос](docs/spros.md) — вопросы и ответы Mail через узкие GET-маршруты nginx. Аккаунты и действия сохраняются в браузере. См. также [описание Морроу](docs/morrow.md). [Fokus](docs/fokus.md) показывает новости РИА, полные статьи, реакции и обсуждения через nginx; локальный просмотр: `python3 scripts/preview_fokus.py`. Замена выполняется с backup и проверкой конфигурации на новом bind mount. Работающий nginx-контейнер точечно пересоздаётся; остановленный пересоздаётся без запуска и проверяется одноразовым контейнером:

```bash
sudo rwm disguise apply 03-morrow-coffee
```

Для локального предпросмотра Морроу нужен его ограниченный API-прокси; VS Code
Live Server показывает интерфейс, но не загружает ленту. Запуск из корня проекта:

```bash
python scripts/preview_morrow.py
```

Пятый сайт [Свод](docs/svod.md) переработан в энциклопедический читатель с RU/EN,
локальной библиотекой и историей. Статьи загружаются из API русской Википедии;
подробности и проверки описаны в документации. Для просмотра
сайта с ограниченным прокси предусмотрен
`python3 scripts/preview_svod.py` (порт 5505). Обычный Live Server эти маршруты
не создаёт.

Шестой сайт [Loop](docs/loop.md) — GIF, стикеры и клипы gifs.ru с RU/EN,
светлой/тёмной темами, локальными аккаунтами, лайками и коллекциями.
API и медиа проходят через nginx; отдельного backend нет. Комментариев нет.
Для локального просмотра: `python3 scripts/preview_loop.py` (порт 5506).
Обычный Live Server не создаёт необходимые API-маршруты.

Для просмотра Спроса с живой лентой нужен его ограниченный прокси:
