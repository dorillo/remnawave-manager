...
document.querySelectorAll('[data-range]').forEach((button) => button.addEventListener('click', () => {
  ...
  const labels = { 24: dateRange(24), 168: dateRange(168), 720: dateRange(720), 2160: dateRange(2160) };
...
const maintenance = {
  export: ['Обновление контура экспорта', `<p><b>${formatDate(ago(-7 * 86400000))}, 02:00–02:30 МСК</b></p><p>Будет обновлён планировщик больших выгрузок. Новые задания останутся в очереди и начнут выполняться после окна. Готовые файлы и остальные компоненты доступны без ограничений.</p>`],
  routing: ['Проверка резервного маршрута', `<p><b>${formatDate(ago(-14 * 86400000))}, 03:00–03:20 МСК</b></p><p>Трафик на короткое время переключится на резервный маршрут. Клиенты с корректной политикой повторов не заметят изменения; отдельный запрос может получить код 503.</p>`]
};
